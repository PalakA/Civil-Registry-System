...
public class DisplayBirth {
	public static List<UserBirth> disp(String fname1, String placename,
			String father) {
		int status = 0;
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("select * from civilreg where firstName='"
							+ fname1 + "' and placeOfBirth='" + placename
							+ "' and fatherName='" + father + "'");
			{
				while (rs.next()) {
