...
public class DisplayDeath {
	public static List<UserDeath> disp(String fname2 ,String placeDeath) {
		ArrayList<UserDeath> list = new ArrayList<UserDeath>();
		try {
			Connection con = ConProv.getCon();
			Statement stmt = con.createStatement();
			ResultSet rs = stmt
					.executeQuery("select * from deathrecord where name='"
							+ fname2 + "'and placeOfDeath='" + placeDeath + "'");
			while (rs.next()) {
